...
appLogin <- function(memberID, currentGPS){
    members_num <-str_which(memberIds,pattern = memberID)
    members$latestGPS <- currentGPS
  str(members[[members_num]])
}
...
map(
  1:200,
  ~{
    creator_name <- painting[[.x]][[1]][[2]]$Creator
    
    creator_name_1 <- str_extract(creator_name,"(?<=[：])[\u4E00-\u9FFF]+")
    str_replace_all(creator_name_1,pattern = " " ,replacement = "無名氏")
  
  }
) ->result
result
allCreators <- unlist(result)
