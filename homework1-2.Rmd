...
`NBA1920Final567` <- list(Game5,Game6,Game7)
...
```{r ans12}
# Game5Time
`Game5Time` <- c(Game5$`Date`,Game5$`Startime`,Game5$`TimeZone`)
print(`Game5Time`)
```
...
```{r ans13}
# G5TimeParsed
paste0(`Game5Time`[[1]]," ",`Game5Time`[[2]])
```
...
```{r ans14}
# TpeJason
Game5Time <- ymd_hm("2020-10-09 21:00",tz="America/New_York")
`TpeJason` <- with_tz(Game5Time, tzone = "Asia/Taipei")
cat(as.character(`TpeJason`))
```
...
```{r ans15}
# ParisLouis
Paris<-with_tz(`NewyorkJason`,tzone="Europe/Paris")
`ParisLouis`<-Paris+minutes(15)
```
...
```{r ans21}
# sumChNumbers
numbers<-as.integer(factor(chNumbers))
sumChNumbers<-sum(numbers)
...
```{r ans22}
# sumChNumbersWith0
numbers1<-as.integer(factor(chNumbersWith0))
sumChNumbersWith0<-sum(numbers1)
```
...
```{r ans31}
# escapeText
aboutEscape <- c("在字串輸入時我們要用成對的\"或\'將文字包起來，R才會知道它是字串。",
"字串在輸入時，若要cat時在螢幕看起來有換行則必需輸入\\n，其中\\稱為Escape", "key，它讓後面字元跳脫了它在電腦的原始含意。")
cat(aboutEscape, sep = "\n")
```
...
```{r ans32}
# dearStudent
time <- paste0(date(student$deadline),"T",hour(student$deadline),":0",minute(student$deadline),":0",second(student$deadline),"+0000")

dearStudent <- paste0("Dear",student$name,":\n\n
貼心提醒您，作業上傳截止時間是",time,"。","\n\n
