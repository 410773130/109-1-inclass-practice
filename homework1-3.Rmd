...
```{r ans12}
# Game5Time
Game5Time <- c(NBA1920Final567$Game5$Date,NBA1920Final567$Game5$StarTime,NBA1920Final567$Game5$TimeZone)
print(Game5Time)
```
...
```{r ans14}
# TpeJason
TpeJason <- with_tz(G5TimeParsed , tz= "Asia/Taipei")
print(TpeJason)
```
...
```{r ans15}
# ParisLouis
ParisLouis <- with_tz(TpeJason , tz="Europe/Paris") + minutes(15)
print(ParisLouis)
```
...
sumChNumberswith0 <- sum(as.integer(fct_chNumberswith0))-length(fct_chNumberswith0)
# sumChNumbersWith0
```
...
```{r ans31}
# escapeText
aboutEscape <- ("在字串輸入時我們要用成對的\"或\'將文字包起來，R才會知道它是字串。
字串在輸入時，若要cat時在螢幕看起來有換行則必需輸入\\n，其中\\稱為Escape
key，它讓後面字元跳脫了它在電腦的原始含意。")
cat(aboutEscape,sep = "\n")
```
...
貼心提醒您，作業上傳截止時間是",format_ISO8601(with_tz(student$deadline,tz="UTC"),usetz=T),"\n\n敬祝學安\n\n你的貼心學伴巴西利bot silly")
cat(dearStudent,sep = "\n")
